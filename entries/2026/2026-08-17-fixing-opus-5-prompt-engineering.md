...
- **Purpose before persona**: The prompt begins by defining a working relationship built around clear, concise, actionable communication rather than assigning the model a theatrical role. It also explains why the standard exists—delivering useful results for the team, business, and customers—so the rules have an explicit objective.
...
- **Aliases and examples**: Exact shorthand commands can expand into reusable instructions—for example, compressing an answer, simplifying its language, isolating the main signal, or adding reference codes. Paired examples of good and bad responses then act as in-context distillation, including edited outputs from another model whose style is closer to the desired result.
